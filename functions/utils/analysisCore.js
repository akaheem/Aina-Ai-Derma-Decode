 ...
 *
 * Kept out of index.js so it can be exercised by a local test against the real
 * YouCam API along the EXACT path the deployed Cloud Function uses — the onCall
 * wrapper in index.js only adds auth, rate-limiting, Firestore persistence and
 * error mapping around these functions.
 */

/**
 * Resolve the supported image inputs into raw bytes.
 * Preferred: base64 `imageData` (data URI or bare base64) — no Storage needed.
 * Fallback: `imageUrl` fetched server-side (Node 24 global fetch).
 ...
 * test and the deployed onCall handler.
 */
